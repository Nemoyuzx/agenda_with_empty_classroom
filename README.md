...
课表数据优先使用北邮移动教务 `sjd` 的课程表接口，自动获取当前教学周并反推第一周周一；如果该接口失败，会回退到新版教务的课表 XLS 解析。

空教室数据只使用北邮移动教务官方服务：按 `restClassroom` 页面对应的 `todayClassrooms` 接口解析实时当天空教室；前端一次获取西土城和沙河两校区并在本地切换展示。该官方实时接口仅支持当天查询，如果登录失败或接口不可用，页面会直接显示错误。
...
如果旧版课表 fallback 的周次不对，检查前端“备用参数”里的“第一周周一”。
